...
const defaultTlsMaxFragmentSize = 16384;
...
Connection.prototype.open = function (callback) {
  ...
  // Use coalescing threshold as socket stream highWaterMark
  const socket = new net.Socket({ highWaterMark: this.options.socketOptions.coalescingThreshold });

  socket.connect(this.port, this.address, function connectCallback() {
    self.log('verbose', 'Socket connected to ' + self.address + ':' + self.port);

    // Remove existing listeners related to socket connect()
    socket.removeAllListeners('connect');
    socket.removeAllListeners('timeout');

    // Set socket level options
    if (self.options.socketOptions.keepAlive) {
      socket.setKeepAlive(true, self.options.socketOptions.keepAliveDelay);
    }
    socket.setNoDelay(!!self.options.socketOptions.tcpNoDelay);

    if (!self.options.sslOptions) {
      self.netClient = socket;
      self.bindSocketListeners();
      self.startup(callback);
    } else {
      const sslOptions = utils.extend({ rejectUnauthorized: false }, self.options.sslOptions, { socket });
      // Perform TLS handshake over the existing socket
      self.netClient = tls.connect(sslOptions, function tlsConnectCallback() {
        self.log('verbose', 'Secure socket connected to ' + self.address + ':' + self.port);
        self.bindSocketListeners();
        self.startup(callback);
      });
      self.netClient.once('error', function socketError(err) {
        self.errorConnecting(err, callback);
      });
      if (self.options.socketOptions.coalescingThreshold > defaultTlsMaxFragmentSize) {
        self.netClient.setMaxSendFragment(self.options.socketOptions.coalescingThreshold);
      }
    }
  });
  ...
  socket.once('timeout', function connectTimedOut() {
    const err = new types.DriverError('Connection timeout');
    socket.destroy(err);
    self.errorConnecting(err, callback);
  });

  socket.setTimeout(this.options.socketOptions.connectTimeout);
};
...
Connection.prototype.startup = function (callback) {
  ...
  function startupCallback(err) {
    if (err) {
      return self.errorConnecting(err, callback);
    }
...
/**
 * Logs a warning and invokes the callback
 * @param {Error} err
 * @param {Function} callback
 */
Connection.prototype.errorConnecting = function (err, callback) {
