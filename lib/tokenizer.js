...
/**
 * Provides Murmur3 hashed values using native BigInt representation.
 */
class Murmur3BigIntTokenizer extends Tokenizer {

  constructor() {
    super();

    this._maxUint64 = BigInt('0xffffffffffffffff');

    // We can't use bigint literals like 0n as it will cause a syntax error on runtimes that don't support bigint.
    this._value0 = BigInt(0);
    this._value5 = BigInt(5);
    this._value8 = BigInt(8);
    this._value16 = BigInt(16);
    this._value24 = BigInt(24);
    this._value27 = BigInt(27);
    this._value31 = BigInt(31);
    this._value32 = BigInt(32);
    this._value33 = BigInt(33);
    this._value40 = BigInt(40);
    this._value48 = BigInt(48);
    this._value56 = BigInt(56);
    this._value64 = BigInt(64);

    this._const1 = BigInt('0x87c37b91114253d5');
    this._const2 = BigInt('0x4cf5ad432745937f');
    this._const3 = BigInt('0xff51afd7ed558ccd');
    this._const4 = BigInt('0xc4ceb9fe1a85ec53');
    this._const5 = BigInt('0x52dce729');
    this._const6 = BigInt('0x38495ab5');
  }
   ...
   */
  rotl64(v, n) {
    return this.toInt64(v << n) | this.toInt64(v >> (this._value64 - n));
  }
  ...
  getBlock(key, offset, index) {
    ...
    return this.toInt64((BigInt(key[blockOffset])) + (BigInt(key[blockOffset + 1]) << BigInt(8)) +
      (BigInt(key[blockOffset + 2]) << BigInt(16)) + (BigInt(key[blockOffset + 3]) << BigInt(24)) +
      (BigInt(key[blockOffset + 4]) << BigInt(32)) + (BigInt(key[blockOffset + 5]) << BigInt(40)) +
      (BigInt(key[blockOffset + 6]) << BigInt(48)) + (BigInt(key[blockOffset + 7]) << BigInt(56)));
  }
  ...
  fmix(k) {
    ...
    k = BigInt.asUintN(64, k * this._const3);
    k ^= (k >> this._value33);

    k = BigInt.asUintN(64, (k * this._const4));
    k ^= (k >> this._value33);

    return this.toInt64(k);
  }
   ...
   */
  hash(value) {
    // This is an adapted version of the MurmurHash.hash3_x64_128 from Apache Cassandra.
    const data = value;
    let offset = 0;
    const length = data.length;
    ...
    for (let i = 0; i < nblocks; i++) {
      let k1 = this.getBlock(data, offset, i * 2);
      let k2 = this.getBlock(data, offset, i * 2 + 1);

      // console.log('k1-pre', k1.toString());

      k1 = (k1 * this._const1) & this._maxUint64;
      k1 = this.rotl64(k1, this._value31);
      k1 = (k1 * this._const2) & this._maxUint64;

      // console.log('k1', k1.toString());

      h1 ^= k1;
      h1 = this.rotl64(h1, this._value27);
      h1 = this.toInt64(h1 + h2);
      h1 = this.toInt64(this.toInt64(h1 * this._value5) + this._const5);

      k2 = (k2 * this._const2) & this._maxUint64;
      k2 = this.rotl64(k2, this._value33);
      k2 = (k2 * this._const1) & this._maxUint64;
      h2 ^= k2;
      h2 = this.rotl64(h2, this._value31);
      h2 = this.toInt64(h2 + h1);
      h2 = this.toInt64(this.toInt64(h2 * this._value5) + this._const6);
    ...
    switch(length & 15) {
      case 15:
        k2 ^= (this.fromSignedByte(data[offset + 14]) << BigInt(48));
      case 14:
        k2 ^= this.fromSignedByte(data[offset + 13]) << this._value40;
      case 13:
        k2 ^= this.fromSignedByte(data[offset + 12]) << this._value32;
      case 12:
        k2 ^= this.fromSignedByte(data[offset + 11]) << this._value24;
      case 11:
        k2 ^= this.fromSignedByte(data[offset + 10]) << this._value16;
      case 10:
        k2 ^= this.fromSignedByte(data[offset + 9]) << this._value8;
      case 9:
        k2 ^= this.fromSignedByte(data[offset + 8]) << this._value0;
        k2 = this.toInt64(k2 * this._const2);
        k2 = this.rotl64(k2 & this._maxUint64, this._value33);
        k2 = this.toInt64(k2 * this._const1);
        h2 ^= k2;
      case 8:
        k1 ^= this.fromSignedByte(data[offset + 7]) << this._value56;
      case 7:
        k1 ^= this.fromSignedByte(data[offset + 6]) << this._value48;
      case 6:
        k1 ^= this.fromSignedByte(data[offset + 5]) << this._value40;
      case 5:
        k1 ^= this.fromSignedByte(data[offset + 4]) << this._value32;
      case 4:
        k1 ^= this.fromSignedByte(data[offset + 3]) << this._value24;
      case 3:
        k1 ^= this.fromSignedByte(data[offset + 2]) << this._value16;
      case 2:
        k1 ^= this.fromSignedByte(data[offset + 1]) << this._value8;
      case 1:
        k1 ^= this.fromSignedByte(data[offset]);
        k1 = this.toInt64(k1 * this._const1);
        k1 = this.rotl64(BigInt.asUintN(64, k1), this._value31);
        k1 = this.toInt64(k1 * this._const2);
        h1 ^= k1;
    }
    /* eslint-enable no-fallthrough */
    ...
    return new token.Murmur3Token(this.toInt64(h1 + h2));
  }

  fromSignedByte(value) {
    return (value < 128) ? BigInt(value) : BigInt(value - 256);
  }
...
exports.Murmur3BigIntTokenizer = Murmur3BigIntTokenizer;
