...
class RequestExecution {
  ...
  _sendOnConnection() {
    ...
    this._operation =
      this._connection.sendStream(this._request, this._parent.executionInfo, function rCb(err, response, length) {
        self._trackResponse(startTime, err, length);
        if (self._cancelled) {
  ...
  _trackResponse(startTime, err, length) {
    ...
    if (err) {
      tracker.onError(this._host, query, parameters, this._parent.executionInfo, requestLength, err, latency);
    } else {
      tracker.onSuccess(this._host, query, parameters, this._parent.executionInfo, requestLength, length, latency);
    }
  }
  ...
  _getDecision(err) {
    const operationInfo = {
      ...
      executionInfo: this._parent.executionInfo,
      nbRetry: this._retryCount
    };

    const retryPolicy = this._parent.executionInfo.getRetryPolicy();
    const self = this;

    function onRequestError() {
      if (operationInfo.executionInfo.getIsIdempotent()) {
        return retryPolicy.onRequestError(operationInfo, self._request.consistency, err);
      }
