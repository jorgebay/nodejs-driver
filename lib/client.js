...
Client.prototype.connect2 = async function (query, params, options, callback) {
  if (this.connected) {
    return;
  }
...
Client.prototype.execute2 = function (query, params, options, callback) {
  return asyncWrapper(callback, async () => {
    await this.connect2();
    ...
    if (!execOptions.isPrepared() && params && !Array.isArray(params) && !types.protocolVersion.supportsNamedParameters(version)) {
    ...
    if (!execOptions.isPrepared()) {
      request = this._createQueryRequest2(query, execOptions, params);
    } else {
      const lbp = execOptions.getLoadBalancingPolicy();

      const { queryId, meta } = await PrepareHandler.getPrepared2(this, lbp, query, this.keyspace);

      request = await this._createExecuteRequest2(query, queryId, execOptions, params, meta);
    }

    return await RequestHandler.send2(request, execOptions, this);
  });
};

async function asyncWrapper(callback, fn) {
  let result;
  let error;
  try {
    result = await fn();
...
Client.prototype._createQueryRequest2 = async function () {
  throw new Error('Not implemented');
};

Client.prototype._createExecuteRequest2 = async function(query, queryId, info, params, meta) {
  params = utils.adaptNamedParamsPrepared(params, meta.columns);

  await this._setRoutingInfo2(info, params, meta);
...
Client.prototype._setRoutingInfo2 = function (execOptions, params, meta) {
  ...
  throw new Error('Not implemented for protocol 3');
};
