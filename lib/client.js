...
const DefaultExecutionInfo = require('./execution-info').DefaultExecutionInfo;
...
Client.prototype.execute = function (query, params, options, callback) {
  ...
  return utils.promiseWrapper.call(this, this.options, callback, function handler(cb) {
    let info;
    try {
      info = DefaultExecutionInfo.create(options, this);
    }
    catch (e) {
      return cb(e);
    }

    this._innerExecute(query, params, info, cb);
  });
};
...
Client.prototype.eachRow = function (query, params, options, rowCallback, callback) {
  ...
  let info;
  try {
    info = DefaultExecutionInfo.create(options, this, rowCallback);
  }
  catch (e) {
    return callback(e);
  }
  ...
  function nextPage() {
    self._innerExecute(query, params, info, pageCallback);
  }
  ...
  function pageCallback (err, result) {
    ...
    if (result.meta && result.meta.pageState) {
      ...
      info.setPageState(result.meta.pageState);
      if (info.getIsAutoPage()) {
  ...
  this._innerExecute(query, params, info, pageCallback);
};
...
Client.prototype._batchCb = function (queries, options, callback) {
  ...
  utils.series([
    ...
    next => {
      if (info.getIsPrepared()) {
        return PrepareHandler.getPreparedMultiple(
          this, info.getExecutionProfile().loadBalancing, queries, this.keyspace, function(err, result) {
            queryItems = result;
            next(err);
          });
      }
    ...
    next => this._createBatchRequest(queryItems, info, (err, r) => {
      request = r;
      next(err);
    }),
    next => RequestHandler.send(request, info, this, next)
  ], callback);
};
 ...
 * @private
 */
Client.prototype._innerExecute = function (query, params, info, callback) {
  const version = this.controlConnection.protocolVersion;

  if (!info.getIsPrepared() && params && !util.isArray(params) && !types.protocolVersion.supportsNamedParameters(version)) {
  ...
  utils.series([
    ...
    next => {
      if (!info.getIsPrepared()) {
        return this._createQueryRequest(query, info, params, (err, r) => {
          request = r;
          next(err);
        });
      }

      const lbp = info.getExecutionProfile().loadBalancing;
      PrepareHandler.getPrepared(this, lbp, query, this.keyspace, (err, queryId, meta) => {
        if (err) {
          return next(err);
        }
        this._createExecuteRequest(query, queryId, info, params, meta, (err, r) => {
          request = r;
          next(err);
        });
      });
 ...
 * @private
 */
Client.prototype._createQueryRequest = function (query, info, params, callback) {
  this.metadata.adaptUserHints(this.keyspace, info.getHints(), (err) => {
    if (err) {
      return callback(err);
    }
    ...
    let paramsInfo;
    try {
      paramsInfo = utils.adaptNamedParamsWithHints(params, info);
      this._getEncoder().setRoutingKeyFromUser(paramsInfo.params, info, paramsInfo.keyIndexes);
    } catch (err) {
      return callback(err);
    }

    callback(null, new requests.QueryRequest(query, paramsInfo.params, info, paramsInfo.namedParameters));
  });
};
 ...
 * @private
 */
Client.prototype._setRoutingInfo = function (info, params, meta, callback) {
  const self = this;
  ...
  function encodeRoutingKey(fromUser) {
    ...
    try {
      if (fromUser) {
        encoder.setRoutingKeyFromUser(params, info);
      } else {
        encoder.setRoutingKeyFromMeta(meta, params, info);
      }
    }
    catch (err) {
      return callback(err);
  ...
  if (!info.getKeyspace() && meta.keyspace) {
    info.setKeyspace(meta.keyspace);
  }
  if (info.getRoutingKey()) {
  ...
  if (Array.isArray(meta.partitionKeys)) {
    ...
    info.setRoutingIndexes(meta.partitionKeys);
  ...
  this.metadata.getTable(meta.keyspace, meta.table, (err, tableInfo) => {
    ...
    if (!tableInfo) {
      // The schema data is not there, maybe it is being recreated, avoid setting the routing info
      return callback();
    }
    info.setRoutingIndexes(tableInfo.partitionKeys.map(c => meta.columnsByName[c.name]));
    // Skip parsing metadata next time
    meta.partitionKeys = info.getRoutingIndexes();
    encodeRoutingKey();
  });
};
