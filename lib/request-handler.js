...
class RequestHandler {
   ...
   */
  constructor(request, info, client) {
    this.client = client;
    this.loadBalancingPolicy = info.getExecutionProfile().loadBalancing;
    this.retryPolicy = info.getRetryPolicy();
   ...
   * @param {Function} callback
   */
  static send(request, info, client, callback) {
    const instance = new RequestHandler(request, info, client);
   ...
   */
  send(callback) {
    if (this.executionInfo.getCaptureStackTrace()) {
      Error.captureStackTrace(this.stackContainer = {});
    }
    this._callback = callback;
    const fixedHost = this.executionInfo.getFixedHost();
    const self = this;
    if (fixedHost) {
      ...
      this.loadBalancingPolicy.newQueryPlan(this.client.keyspace, this.executionInfo, function newPlanCb(err, iterator) {
        if (err) {
  ...
  _startNewExecution() {
    ...
    execution.start(function hostAcquired(host) {
      ...
      if (!self.executionInfo.getIsIdempotent()) {
        return;
      }
